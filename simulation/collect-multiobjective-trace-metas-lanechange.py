...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-lanechange3.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def superdeepbillboard(model, sequence, direction, steering_vector, bb_size=5, iterations=400, noise_level=25,
    ...
    if dist_to_bb is not None and facing_target is not None and not facing_target['facing']:
        constraint = 0.167 #dist_to_bb / 22.0 #math.atan(dist_to_bb / 22.0) # 0.2 # 0.1667
...
def run_scenario_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
    ...
    for i in range(10):
        runresults = run_scenario_with_perturbed_billboard(vehicle, bng, model, spawn, pert_billboards[-1],
                                                           ...
                                                           dist_to_bb_cutoff=dist_to_bb_cutoff, goalseq=goalseq)
...
def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda'), goalseq=None):
...
def run_scenario_with_perturbed_billboard(vehicle, bng, model, spawn, pert_billboard, run_number=0, device=torch.device('cuda'),
    ...
    while damage <= 0:
        ...
        percent_of_img = get_percent_of_image(qr_corners, image)
        try:
            dist_to_goal = distance2D(vehicle.state['front'], goalseq[goal_index])
        except:
            goal_index = len(goalseq)-1
        ...
        if kph > 29:
            ...
            road_seg = nearest_seg(roadmiddle, vehicle.state['front'])
            x = intersection_of_RS_and_road(rs, road_seg)
            plot_facing_bb_with_CV2(vehicle.state, rs, road_seg, round(x, 2), vehicle.get_bbox(), curr_goal=goalseq[goal_index], traj=traj)
        ...
        if damage > 0.0:
            ...
            if goal_index >= len(goalseq) - 1:
                print("GOALS MET. Returned to normal trajectory, exiting...")
...
def car_facing_target(vehicle_state, vehicle_bbox, goal):
    ...
    temp2 = math.degrees(find_angle_atan(vehicle_state['front'], car_back_center, goal))
    alphas = abs(round(temp2 - 180.0, 0)) < 4
    print(f"{temp2=}")
    print(f"{alphas=}")
    return {"facing":alphas, "max":temp2, "min": temp2}
...
def run_scenario_for_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
    ...
    while damage <= 0:
        ...
        if round(dist_to_goal, 0) < 1.0 or vehicle.state['front'][1] > goalseq[goal_index][1]:
            if goal_index == len(goalseq) - 1:
                pass
            else:
                goal_index += 1
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton:
            ...
            road_seg = nearest_seg(roadmiddle, vehicle.state['front'])
            x = intersection_of_RS_and_road(rs, road_seg)
            plot_facing_bb_with_CV2(vehicle.state, rs, road_seg, round(x, 2), vehicle.get_bbox(), curr_goal=goalseq[goal_index])
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton:
            ...
            facing_target = car_facing_target(vehicle.state, vehicle.get_bbox(), goalseq[goal_index])
...
def main():
    ...
    steps_per_sec = 15
    samples = 20
    goalseq = [[208.135,-169.540],[208.016, -166],[203,-150.619]]
    goalseq = [[209.36,-169.540],[203.533,-150.619]]
    vehicle, bng, model, spawn = setup_beamng(setup_args, vehicle_model='hopper', model_name=model_name)
    newdir = "multiobjective-lanechange-constraint0.167-{}".format(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
