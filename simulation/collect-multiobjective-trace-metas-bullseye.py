...
default_spawnpoint = "startinggate"
...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-bullseye2.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
            pos = tuple([float(i) for i in pos])
            rot_quat = line[1].split(',')
            rot_quat = [float(j) for j in rot_quat]
        ...
        if default_scenario == "industrial" and default_spawnpoint == "curve1":
            cube = ProceduralCube(name='cube_platform',
                                  pos=(145.214,-160.72,43.7269), rot=None,
                                  rot_quat=(0, 0, 0, 1), size=(2, 6, 0.5))
            scenario.add_procedural_mesh(cube)
        elif default_scenario == "industrial" and default_spawnpoint == "startinggate":
            cube = ProceduralCube(name='cube_platform',
                                  #pos=(225, -50, 42), rot=None,
                                  #rot_quat=(0, 0, 0, 1), size=(6, 6, 2))
                                  pos=(225, -46, 43), rot = None,
                                  # rot_quat = (-0.03237, 0.0003204, 0.34141, 0.9394), size = (6, 6, 0.5))
                                  rot_quat=(-0.03237, 0.0003204, 0.34141, 0.9394), size=(4, 6, 0.25))
            scenario.add_procedural_mesh(cube)
        elif default_scenario == "driver_training" and default_spawnpoint == "approachingfork":
            cube = ProceduralCube(name='cube_platform',
                                  pos=(-20.3113, 218.448, 50.043), rot=None,
...
def plot_deviation(trajectories, unperturbed_traj, model, centerline, left, right, outcomes,
    ...
    if default_spawnpoint == "straight1" and "ZOOMED" not in model:
        ...
        plt.xlim([325, 345])
        plt.ylim([-120, -100])
    elif default_spawnpoint == "startinggate":
        plt.xlim([150, 250])
        plt.ylim([-125, -25])
    elif default_scenario == "driver_training" and default_spawnpoint == "approachingfork":
        plt.xlim([-50, 55])
...
def road_analysis(bng):
    ...
    industrial_racetrack_locations = ["startinggate", "straight1"]
    if default_scenario == "industrial" and default_spawnpoint in industrial_racetrack_locations:
...
@ignore_warnings
def plot_intersection_with_CV2(vehicle_state, rs, road_seg, intersection, bbox, curr_goal=None):
    global qr_positions
    fig, ax = plt.subplots()
    # print(f"{bbox=}")
    ...
    if curr_goal is not None:
        plt.plot([curr_goal[0]], [curr_goal[1]], linewidth=5, label="curr_goal")
    # add road segment
...
def run_scenario_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
                                    collect_sequence_results=None, goalseq=[],
...
def run_scenario_for_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
                                        bb_size=5, iterations=100, noise_level=25,  goalseq = [],
...
def main():
    ...
    steps_per_sec = 15
    samples = 25
    # goalseq = [[194.0,-65.0855],[197.51575,-62.720825],[201.0315,-60.35615],[204.547,-57.991],[208.063,-55.6268],[210.396,-52.0526],[211.56,-50.266],[212.729,-48.4784],[220, -43]] #[212.313,-50.9713]]
    goalseq = [[204.55, -57.99], [207.5, -54.63], [210.4, -51.5], [211, -49.75], [212.73, -47.48],
               [220, -43]]
    #goalseq = [[207.5, -54.63], [220, -43]]

    vehicle, bng, model, spawn = setup_beamng(setup_args, vehicle_model='hopper', model_name=model_name)
    newdir = "multiobjective-bullseye-{}".format(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
