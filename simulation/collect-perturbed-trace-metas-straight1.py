...
default_spawnpoint = "straight1" #"approachingfork"  # "curve1" #
...
def spawn_point(scenario_locale, spawn_point='default'):
    ...
    if scenario_locale == 'industrial':
        ...
        if spawn_point == "curve1":
            ...
            return {'pos': (184.971,-73.653,42.9918), 'rot': None,
...
def make_results_dirs(newdir, technique, bbsize, nl, its, cuton, rsc, timestr):
    new_results_dir = "results/{}/results-{}-{}-{}-{}-cuton{}-rs{}-{}-{}".format(newdir, technique, bbsize, nl, its, cuton, rsc, timestr, ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
...
def main():
    ...
    steps_per_sec = 15
    samples = 25
    newdir = f"Exp1-equalinfo-nofullseq-{default_spawnpoint}-{''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}"
                            ...
                            for i in range(samples):
                                ...
                                if technique == "dbb":
                                    ...
                                    results = run_scenario(vehicle, bng, model, spawn, direction, collect_sequence_results=unperturbed_results,
                                                                              bb_size=bbsize, iterations=its, noise_level=1000, dist_to_bb_cuton=cuton,
