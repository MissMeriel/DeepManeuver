...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-curve4-3x3.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def main():
    ...
    steps_per_sec = 15
    samples = 35
    newdir = f"sdbb-addexptrajend-constraint1-{default_spawnpoint}-{''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}"
