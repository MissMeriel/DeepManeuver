...
def spawn_point(scenario_locale, spawn_point='default'):
    ...
    if scenario_locale == 'industrial':
        ...
        if spawn_point == "curve1":
            ...
            rot_quat = turn_90((0.0095777017995715, 0.033657912164927, -0.35943350195885, 0.93251436948776))
            rot_quat = (0.0062633226625621,0.023590445518494,0.9978541135788,0.060758579522371)
            return {'pos': (215.2,-205.4,45.5), 'rot': None, 'rot_quat': rot_quat}
    ...
    with open('posefiles/qr_box_locations-cutcorner.txt', 'r') as f:
        ...
        if default_scenario == "industrial" and default_spawnpoint == "curve1":
            cube = ProceduralCube(name='cube_platform',
                                  # pos=(259,-17.25,43.25),
                                  # pos=(259, -17.125, 43.25),
                                  pos=(256.5,-18, 43.25),
                                  rot=None,
                                  rot_quat=(0.00202, 0.00498, 0.9202, -0.3914),
                                  size=(4, 6, 2))
            scenario.add_procedural_mesh(cube)
        elif default_scenario == "industrial" and default_spawnpoint == "straight2":
            rot_quat = (0.0062633226625621,0.023590445518494,0.9978541135788,0.060758579522371)
            print(f"{rot_quat=}")
            cube = ProceduralCube(name='cube_platform',
                                  pos=(211.5,-150.619,43.5),
                                  rot=None,
                                  rot_quat=rot_quat,
                                  size=(6, 6, 2))
            scenario.add_procedural_mesh(cube)
...
def plot_deviation(trajectories, unperturbed_traj, model, centerline, left, right, outcomes,
    ...
    failures, bullseyes, goals = 0, 0, 0
    for o in outcomes:
        if o == "LT" or "D" in o:
            failures += 1
        if "BULLSEYE" in o:
        ...
        if "GOAL" in o:
            goals += 1
    plt.title('Trajectories with {} failures={} goals={}'.format(model, failures, goals), fontdict={'fontsize': 10})
    # plt.legend()
    if default_spawnpoint == "straight1" and "ZOOMED" not in model:
        ...
        plt.xlim([325, 345])
        plt.ylim([-120, -100])
    elif default_spawnpoint == "straight2":
        plt.xlim([175, 225])
        plt.ylim([-199, -149])
    elif default_scenario == "driver_training" and default_spawnpoint == "approachingfork":
        plt.xlim([-50, 55])
...
def road_analysis(bng):
    ...
    industrial_racetrack_pts = ["startinggate", "straight1", "straight2", "curve1", "curve2"]
    if default_scenario == "industrial" and default_spawnpoint in industrial_racetrack_pts:
...
@ignore_warnings
def plot_intersection_with_CV2(vehicle_state, rs, road_seg, intersection, bbox, curr_goal=None, traj=None):
    global qr_positions
    fig, ax = plt.subplots()
    # print(f"{bbox=}")
    ...
    radius = 11.1
    if traj is not None:
        plt.plot([p[0] for p in traj], [p[1] for p in traj], "tab:blue", label="trajectory")
...
@ignore_warnings
def plot_facing_bb_with_CV2(vehicle_state, rs, road_seg, intersection, bbox, curr_goal=None, traj=None):
    global qr_positions
    fig, ax = plt.subplots()
    # print(f"{bbox=}")
    # print(f"{vehicle_state=}")
...
def run_scenario_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
                                    collect_sequence_results=None, goalseq=None,
    ...
    pert_billboards, perturbation_run_results = run_scenario_for_superdeepbillboard_target(vehicle, bng, model, spawn, direction,
                                                                               bb_size=bb_size, iterations=iterations, noise_level=noise_level, goalseq=goalseq,
...
def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda')):
    ...
    while damage <= 0:
        ...
        dist_to_bb = distance2D(vehicle.state['front'],qrbox_pos)
...
def run_scenario_with_perturbed_billboard(vehicle, bng, model, spawn, pert_billboard, run_number=0, device=torch.device('cuda'),
                                           dist_to_bb_cuton=None, dist_to_bb_cutoff=None, goalseq=None):
    ...
    goal_reached = np.zeros(len(goalseq))
    goalindex = 0
    outcomestring = ''
    all_ys = []
    ...
    while damage <= 0:
        ...
        image = sensors['front_cam']['colour'].convert('RGB')
        colorseg_img = sensors['front_cam']['annotation'].convert('RGB')
        viz_colorseg_image = np.array(colorseg_img).astype(np.uint8)
        viz_colorseg_image = cv2.resize(viz_colorseg_image, (420, 236)) #(240, 135)
        cv2.imshow('semseg view', viz_colorseg_image[:, :, ::-1])
        dist_to_bb = distance2D(vehicle.state['front'],qrbox_pos)
        qr_corners, bbox_img = get_qr_corners_from_colorseg_image(colorseg_img)
        # qr_corners, bbox_img = get_qr_corners_from_colorseg_image_nowarp(colorseg_img)
        percent_of_img = get_percent_of_image(qr_corners, image)
        ...
        if bbox_img is not None and kph > 29:
            viz_image = np.array(image_pert).astype(np.uint8)
            viz_image = cv2.resize(viz_image, (420, 236))  # (240, 135)
            cv2.imshow('car view', viz_image[:, :, ::-1])
            cv2.waitKey(1)
        else:
            viz_image = np.array(origimage).astype(np.uint8)
            viz_image = cv2.resize(viz_image, (420, 236))  # (240, 135)
            cv2.imshow('car view', viz_image[:, :, ::-1])
            cv2.waitKey(1)
        ...
        # STOPPING CONDITIONS
        try:
            dist = distance2D(goalseq[goalindex], vehicle.state['front'])
        except:
            goalindex = len(goalseq)-1
            dist = distance2D(goalseq[goalindex], vehicle.state['front'])
        #print(f"Distance from goalseq[{goalindex} = {dist:.2f}]")
        if dist < 3:
            goal_reached[goalindex] = True
            print(f"{goalindex}th goal reached")
            goalindex += 1
        ...
        if kph > 29:
            ...
            road_seg = nearest_seg(roadmiddle, vehicle.state['front'])
            x = intersection_of_RS_and_road(rs, road_seg)
            plot_facing_bb_with_CV2(vehicle.state, rs, road_seg, round(x, 2), vehicle.get_bbox(),
                                    curr_goal=goalseq[goalindex], traj=traj)
        ...
        if damage > 0.0:
            ...
            if goal_reached.all():
                outcomestring = "GOAL-REACHED"
                print("Goal reached; Returned to normal trajectory, exiting...")
                break
            outcomestring = "R2NT"
...
def car_facing_target(vehicle_state, vehicle_bbox, goal):
    global qr_positions
    car_back_center = [(vehicle_bbox["rear_bottom_left"][0] + vehicle_bbox["rear_bottom_right"][0])/2,
                       (vehicle_bbox["rear_bottom_left"][1] + vehicle_bbox["rear_bottom_right"][1])/2,
                       vehicle_bbox["rear_bottom_right"][2]]
    alphas = np.zeros(len(qr_positions))
    # alphas2 = np.zeros(len(qr_positions))
    # temps = np.zeros(len(qr_positions))
    temps2 = np.zeros(len(qr_positions))
    for i,p in enumerate(qr_positions):
        temp = law_of_cosines(vehicle_state['front'], car_back_center, goal)
        temp2 = find_angle_atan(vehicle_state['front'], car_back_center, goal)
        # temps[i] = math.degrees(temp)
        temps2[i] = math.degrees(temp2)
        alphas[i] = abs(round(math.degrees(temp) - 180.0, 0)) < 4
        # alphas2[i] = abs(math.degrees(temp2)) < 4
    return {"facing":alphas.any(), "max":max(temps2), "min":min(temps2)}
...
def run_scenario_for_superdeepbillboard_target(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
                                        bb_size=5, iterations=100, noise_level=25, goalseq=None,
    ...
    while damage <= 0:
        ...
        last_steering_from_sim = sensors["electrics"]["steering_input"]
        dist_to_bb = distance2D(vehicle.state['front'], qrbox_pos)
        dist_to_goal = distance2D(vehicle.state['front'], curr_goal)
        kph = ms_to_kph(sensors['electrics']['wheelspeed'])
        damage = sensors['damage']['damage']
        runtime = sensors['timer']['time'] - start_time
        origimage = sensors['front_cam']['colour'].convert('RGB')
        ...
        if round(dist_to_goal, 0) < 1.0 or vehicle.state['front'][1] > curr_goal[1]:
            if goal_index == len(goalseq) - 1:
                pass
            else:
                goal_index += 1
                curr_goal = goalseq[goal_index]
        ...
        if kph > 29:
            ...
            x = intersection_of_RS_and_road(rs, road_seg)
            plot_facing_bb_with_CV2(vehicle.state, rs, road_seg, round(x, 2), vehicle.get_bbox(),
                                    curr_goal=goalseq[goal_index], traj=traj)
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton:
            ...
            road_seg = nearest_seg(roadmiddle, vehicle.state['front'])
            x = intersection_of_RS_and_road(rs, road_seg)
            plot_facing_bb_with_CV2(vehicle.state, rs, road_seg, round(x, 2), vehicle.get_bbox(), curr_goal=curr_goal)
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton:
            ...
            facing_target = car_facing_target(vehicle.state, vehicle.get_bbox(), curr_goal)
            ...
            # cv2.waitKey(1)
        elif kph > 29: #bbox_img is not None and kph > 29:
            sequence.append({"image": model.process_image(image)[0], "bbox": qr_corners,
                             "colorseg_img": model.process_image(colorseg_img)[0]})
            steering_vector.append(last_steering_from_sim)

        viz_image = np.array(image).astype(np.uint8)
        viz_image = cv2.resize(viz_image, (420, 236))  # (240, 135)
        cv2.imshow('car view', viz_image[:, :, ::-1])
        cv2.waitKey(1)
...
def main():
    ...
    steps_per_sec = 15
    samples = 25
    # goalseq = [[245,-24],[240,-30],[250,-50],[257.4,-28]]
    goalseq = [[245, -24]]
    vehicle, bng, model, spawn = setup_beamng(setup_args, vehicle_model='hopper', model_name=model_name)
    newdir = "multiobjective-bullseye-constraint0.2-{}".format(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
                            ...
                            for lossfxn in lossfxns:
                                for i in range(samples):
                                    ...
                                    results = run_scenario_superdeepbillboard_target(vehicle, bng, model, spawn, direction, collect_sequence_results=unperturbed_results,
                                                                              bb_size=bbsize, iterations=its, noise_level=nl, dist_to_bb_cuton=cuton, goalseq=goalseq,
