...
class Demo1():
    ...
    def __init__(self):
        self.default_scenario = "industrial"
        self.default_spawnpoint = "straight1"
        self.integral = 0.0
        self.prev_error = 0.0
        self.overall_throttle_setpoint = 40.0
        self.setpoint = self.overall_throttle_setpoint
        self.unperturbed_traj = []
        self.unperturbed_steer = []
        self.newdir = ""
        self.new_results_dir = ""
        self.qr_positions = []
        self.unperturbed_seq = None


    def ignore_warnings(self, f):
        @wraps(f)
    ...
    def plot_deviation(self, trajectories, unperturbed_traj, model,
    ...
    @ignore_warnings
    def write_results(self, training_file, results, all_trajs, unperturbed_traj, modelname, technique, direction, lossname, bbsize, its, nl):
        """Pickle results."""
    ...
    def calc_points_of_reachable_set(self, vehicle_state):
    ...
    def intersection_of_RS_and_road(self, rs, road_seg):
    ...
    @ignore_warnings
    def plot_intersection_with_CV2(self, vehicle_state, rs, road_seg, intersection, bbox, traj=None):
    ...
    def plot_intersection(self, vehicle_state, rs, road_seg, intersection, bbox):
        """."""
        fig, ax = plt.subplots()
        yaw = vehicle_state["yaw"][0]
        patches = []
    ...
    def is_billboard_fully_viewable(self, image, qr_corners):
    ...
    def deepbillboard(self, model, sequence, direction, device=torch.device("cuda"),
    ...
    def get_percent_of_image(self, coords, img):
    ...
    def run_scenario(self, sim, model, direction, bb_size=5, iterations=400, noise_level=25, dist_to_bb_cuton=37, resultsdir="images", input_divers=False):
        ...
        starttime = time.time()
        sequence, unperturbed_results = self.run_scenario_to_collect_sequence(sim, model, cuton=dist_to_bb_cuton)
    ...
    def run_scenario_deepmaneuver(self, sim, model, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, device=torch.device("cuda"),
        ...
        starttime = time.time()
        pert_billboards, perturbation_run_results = self.run_scenario_for_deepmaneuver(
            sim,
            model,
            direction,
            device=device,
    ...
    def run_scenario_to_collect_sequence(self, sim, model, cuton=40, device=torch.device("cuda")):
    ...
    def run_scenario_with_perturbed_billboard(self, sim, model, pert_billboard, dist_to_bb_cuton=None, dist_to_bb_cutoff=None):
    ...
    def car_facing_billboard(self, vehicle_state):
        global qr_positions
    ...
    def run_scenario_for_deepmaneuver(self, sim, model, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26, bb_size=5,
    ...
    def make_results_dirs(self, newdir, technique, bbsize, nl, its, cuton, rsc, input_div=None, timestr=None):
                            ...
                            for technique in techniques:
                                for input_div in input_divs:
                                    for i in range(samples):
                                        ...
                                        if technique == "dbb":
                                            lossname = "MDirE"
                                            new_results_dir, training_file = self.make_results_dirs(
                                            ...
                                            results = self.run_scenario_deepmaneuver(
                                                sim, model, direction,
                                                device=device, collect_sequence_results=unperturbed_results,
                                                bb_size=bbsize, iterations=its, noise_level=nl,
                                        ...
                                        self.write_results(training_file, results, all_trajs, unperturbed_traj, model._get_name(),
                                                        technique, direction, lossname, bbsize, its, nl
                                        )
                                        self.plot_deviation(all_trajs, unperturbed_traj,
