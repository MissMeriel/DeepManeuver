...
def plot_deviation(trajectories, unperturbed_traj, model,
                   sim, outcomes, resultsdir="images"):
    """Plot trajectories over unperturbed run, generation run, and test runs."""
    # global qr_positions .centerline_interpolated, sim.roadleft, sim.roadright
    x = [point[0] for point in unperturbed_traj]
    y = [point[1] for point in unperturbed_traj]
    plt.plot(x, y, label="Unpert", linewidth=10)
    x, y = [], []
    for point in sim.centerline_interpolated:
        x.append(point[0])
        y.append(point[1])
    x.append(sim.centerline_interpolated[0][0])
    y.append(sim.centerline_interpolated[0][1])
    plt.plot(x, y, "k")
    x, y = [], []
    for point in sim.roadleft:
        x.append(point[0])
        y.append(point[1])
    plt.plot(x, y, "k")
    x, y = [], []
    for point in sim.roadright:
        x.append(point[0])
        y.append(point[1])
    plt.plot(x, y, "k")
    x, y = [], []
    for i, t in enumerate(zip(trajectories, outcomes)):
        ...
        i += 1
    plt.plot([p[0][0] for p in sim.qr_positions], [p[0][1] for p in sim.qr_positions], "r", linewidth=5)
    failures = 0
    for o in outcomes:
        if o == "LT" or "D" in o:
            failures += 1
    ...
    if default_spawnpoint == "straight1" and "ZOOMED" not in model:
        ...
        plt.xlim([-50, 55])
        plt.ylim([150, 255])
    elif default_spawnpoint == "8205" and default_scenario == "automation_test_track":
        plt.xlim([500, 610])
        plt.ylim([150, 260])
    ...
    while damage <= 0:
        ...
        with torch.no_grad():
            ...
            if bbox_img is not None and kph > 29:
                ...
                cv2.imshow("car view", np.array(image, dtype=np.uint8)[:, :, ::-1])
                cv2.waitKey(1)
...
def run_scenario_for_deepmaneuver(sim, model, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26, bb_size=5,
    ...
    while damage <= 0:
        ...
        sim.bng.step(1, wait=True)
    cv2.destroyAllWindows()
    print(f"{len(unperturbed_predictions)=}\n{len(perturbed_predictions)=}")
    mse = mean_squared_error(unperturbed_predictions, perturbed_predictions)
...
def main():
    ...
    sim = Simulator(
        ...
        steps_per_sec=15,
        bb_filename="posefiles/qr_box_locations-8205-swerve0.txt",
    )
    samples = 3
                                ...
                                for i in range(samples):
                                    ...
                                    plot_deviation(all_trajs, unperturbed_traj,
                                                   f"{technique}-{direction}-{lossname}\nDOF{bbsize}-noisevar{nl}-cuton{cuton}",
                                                    sim, all_outcomes, resultsdir=new_results_dir,
                                    )
    sim.bng.close()
