...
import sys, shutil, logging, random, string, time, copy, os
...
default_spawnpoint = "scenario3" #"topo21"
integral = prev_error = 0.0
overall_throttle_setpoint = 40.0 #https://en.wikipedia.org/wiki/Speed_limits_by_country
...
def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda')):
    global newdir, new_results_dir, steps_per_sec, expected_trajectory, unperturbed_steer, roadmiddle, unperturbed_seq
    global integral, prev_error, setpoint
...
def run_scenario_for_deepmaneuver(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
    ...
    global newdir, new_results_dir, centerline, expected_trajectory, qr_positions
...
def main():
    ...
    global prev_error, centerline, centerline_interpolated, unperturbed_traj, direction, expected_trajectory
    global expected_trajectory, steps_per_sec
    start = time.time()
    direction = "left"
    techniques = ["dbb-plus", "sdbb"]
