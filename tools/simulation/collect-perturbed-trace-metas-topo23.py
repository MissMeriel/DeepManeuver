...
default_spawnpoint = "curve4" #"straight1" #"approachingfork"  # "curve1" #
...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-topo2.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def superdeepbillboard(model, sequence, direction, steering_vector, bb_size=5, iterations=400, noise_level=25,
    ...
    if dist_to_bb is not None:
        ...
        if direction == "left":
            steering_vector.append(-constraint)  # steering_vector[-1]-0.1
        if direction == "straight":
            steering_vector.append(0)  # steering_vector[-1]-0.1
        else:
            steering_vector.append(constraint)  # steering_vector[-1]+0.1
...
def main():
    ...
    direction = "straight"
    techniques = ["sdbb", "dbb-plus", "dbb-orig"]
    ...
    steps_per_sec = 15
    samples = 35
    newdir = f"TSErev-straightpert-{default_spawnpoint}-{''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}"
