...
default_spawnpoint = "topo21" #"straight1" #"approachingfork"  # "curve1" #
...
def spawn_point(scenario_locale, spawn_point='default'):
    ...
    if scenario_locale == 'industrial':
        ...
        elif spawn_point == "topo21":
            return {'pos': (170.179,-111.787,44.6669), 'rot': None, 'rot_quat': (-0.023691736161709, -0.028266951441765, 0.14190191030502, 0.989193379879)}
...
def add_qr_cubes(scenario):
    global qr_positions
    qr_positions = []
    with open('posefiles/qr_box_locations-topo21.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
...
def road_analysis(bng):
    ...
    elif default_scenario == "industrial" and (default_spawnpoint == "straight1" or default_spawnpoint == "curve2" or default_spawnpoint == "curve1" or default_spawnpoint == "curve4" or default_spawnpoint == "topo21"):
...
def run_scenario_superdeepbillboard(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
                                    collect_sequence_results=None,
                                    bb_size=5, iterations=400, noise_level=25, resultsdir="images", input_divers=True, loss_fxn='inv23', add_exp_traj=False):
    ...
    pert_billboards, perturbation_run_results = run_scenario_for_superdeepbillboard(vehicle, bng, model, spawn, direction,
                                                                                ...
                                                                                input_divers=input_divers, loss_fxn=loss_fxn, add_exp_traj=add_exp_traj)
    timetorun = time.time() - starttime
...
def run_scenario_for_superdeepbillboard(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
                                        ...
                                        device=torch.device('cuda'), add_exp_traj=False):
    ...
    results = {'runtime': round(runtime, 3), 'damage': damage, 'kphs': kphs, 'traj': traj, 'final_img': final_img,
               ...
               "outcome":outcome, "all_ys": all_ys, "num_billboards": len(pert_billboards), "MAE": MAE, "seq_len": seq_len
               }
...
def main():
    ...
    steps_per_sec = 15
    samples = 50
    newdir = f"TSErev-topo22straightpert-{default_spawnpoint}-{''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}"
                            ...
                            for input_div in input_divs:
                                for i in range(samples):
                                    ...
                                    if technique == "dbb":
                                        ...
                                        results = run_scenario(vehicle, bng, model, spawn, direction,
                                                               collect_sequence_results=unperturbed_results,
                                                               bb_size=bbsize, iterations=its, noise_level=15,
                                                               dist_to_bb_cuton=cuton,
                                        ...
                                        results = run_scenario_superdeepbillboard(vehicle, bng, model, spawn, direction, collect_sequence_results=unperturbed_results,
                                                                                  ...
                                                                                  dist_to_bb_cutoff=rsc, resultsdir=new_results_dir, input_divers=False, loss_fxn=lossname, add_exp_traj=True)
