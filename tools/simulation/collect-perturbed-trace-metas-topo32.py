...
def spawn_point(scenario_locale, spawn_point='default'):
    ...
    if scenario_locale == 'industrial':
        ...
        elif spawn_point == "leftcurve":
            return {"pos": (213.948, -192.782, 44.9482), "rot":None, "rot_quat": (0.0057079819962382, 0.022676283493638, 0.98823064565659, 0.15117366611958)}
            # return {"pos":(205.624, -162.703, 44.1363), "rot":None, "rot_quat": (0.0022462382912636, 0.01971354149282, 0.99168574810028, 0.12714475393295)}
...
def add_qr_cubes(scenario):
    ...
    with open('posefiles/qr_box_locations-topo32.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            # pos = line[0].split(',')
            pos = tuple([float(k) for k in line[0].split(',')])
            # rot_quat = line[1].split(',')
            rot_quat = tuple([float(j) for j in line[1].split(',')])
            ...
            cube = ProceduralCube(name='cube_platform',
                                  pos=(153.644,-175.733,44.3893),
                                  rot=None,
                                  # 289.912|E|libbeamng.lua.V.updateGFX|Object position: vec3(153.644,-175.733,44.3893)
                                  # 289.913|E|libbeamng.lua.V.updateGFX|Object rotation (quat): quat(-0.057561937719584,-0.017708277329803,-0.42818936705589,0.90168005228043)
                                  rot_quat=(-0.057561937719584,-0.017708277329803,-0.42818936705589,0.90168005228043),
                                  size=(2.5, 5, 1.5))
...
def plot_deviation(trajectories, unperturbed_traj, model, centerline, left, right, outcomes,
    ...
    if default_spawnpoint == "straight1" and "ZOOMED" not in model:
        ...
        plt.xlim([325, 345])
        plt.ylim([-120, -100])
    elif default_spawnpoint == "leftcurve":
        plt.xlim([150, 210])
        plt.ylim([-160, -100])
...
def run_scenario(vehicle, bng, model, spawn, direction, run_number=0,
                                        ...
                                        bb_size=10, iterations=400, noise_level=15, dist_to_bb_cuton=40, resultsdir="images", input_divers=False):
...
def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda')):
    ...
    while damage <= 0:
        ...
        dist_to_bb = distance2D(vehicle.state['pos'],qrbox_pos)
        ...
        if bbox_img is not None and kph > 29 and dist_to_bb < cuton:
            ...
            imagecount += 1
        if kph > 29 and bbox_img is not None and not is_billboard_fully_viewable(image, qr_corners):
...
def run_scenario_with_perturbed_billboard(vehicle, bng, model, spawn, pert_billboard, run_number=0, device=torch.device('cuda'),
    ...
    while damage <= 0:
        ...
        if damage > 0.0:
            ...
            break
        elif distance(qrbox_pos, vehicle.state['pos']) > 35 and runtime > 10:
            outcomestring = "2FAR"
...
def main():
    ...
    direction = "straight"
    techniques = ["dbb-orig", "dbb-plus", "sdbb"] #[["sdbb", "dbb-orig", ] ,
    ...
    steps_per_sec = 15
    samples = 50
    newdir = f'EXP1-TOPOR3-{default_spawnpoint}-{"".join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}'
    if not os.path.isdir("results/{}".format(newdir)):
        os.mkdir("results/{}".format(newdir))
        print(f"Copying script to {__file__}")
        shutil.copy(f"{__file__}", f"results/{newdir}")
        shutil.copy(f"{os.getcwd()}/../deepbillboard/SuperDeepBillboard.py", f"results/{newdir}")
        shutil.copy(f"{os.getcwd()}/../deepbillboard/DeepBillboard.py", f"results/{newdir}")
