...
def run_scenario(vehicle, bng, model, spawn, direction, run_number=0, collect_sequence_results=None,
...
def run_scenario_for_deepmaneuver(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
    ...
    while damage <= 0:
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton: # and (steps % 2 < 1):
            ...
            cv2.imwrite(f"H:/Adv4Trenton/im-adv-{len(pert_billboards):04d}.jpg", image[:, :, ::-1])
            cv2.imwrite(f"H:/Adv4Trenton/im-orig-{len(pert_billboards):04d}.jpg", np.array(origimage)[:, :, ::-1])
            cv2.imshow('car view', image[:, :, ::-1])
            cv2.waitKey(1)

            with torch.no_grad():
                deviceimg = model.process_image(np.asarray(image)).to(device)
                prediction = model(deviceimg).cpu().item()
                deviceimgorig = model.process_image(np.asarray(origimage)).to(device)
                predictionorig = model(deviceimgorig).cpu().item()

            with open("H:/Adv4Trenton/data.csv", "a") as f:
                f.write(f"im-orig-{len(pert_billboards):04d}.jpg,{predictionorig},im-adv-{len(pert_billboards):04d}.jpg,{prediction}\n")
...
def main():
    ...
    global prev_error, centerline, centerline_interpolated, unperturbed_traj, expected_trajectory
    global steps_per_sec
    start = time.time()
    direction = "left"
    techniques = ["sdbb", "dbb", "dbb-plus", ]
    ...
    steps_per_sec = 15
    samples = 2
    newdir = "scenario1-{}".format(''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
