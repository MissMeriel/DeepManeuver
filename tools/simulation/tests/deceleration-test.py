...
default_spawnpoint = 'deceleration_test'
integral = 0.0
prev_error = 0.0
...
def plot_trajectory(traj, addtl_trajs=None, title="Deceleration Trajectory", label1="car traj."):
    global track
    ...
    if addtl_trajs is not None:
        for t in addtl_trajs:
            x = [p[0] for p in t]
            y = [p[1] for p in t]
            plt.plot(x, y, 'r--', label="deceleration")
...
def run_test(vehicle, bng, scenario, model, vehicle_model='etk800', run_number=0,
    ...
    throttle = 0.0; prev_error = throttle_setpoint
    ...
    while current_time <= 60:
        ...
        if abs(kph - throttle_setpoint) > 2 and not start_measuring:
            vehicle.control(throttle=throttle, steering=0.0, brake=0.0)
            orig_pos = vehicle.state['pos']
            deceleration_start_time = copy.deepcopy(runtime)
        else:
        ...
        if start_measuring:
            decel_traj.append(vehicle.state['pos'])
        
        vehicle.update_vehicle()
        # curr_yaw = vec_to_rpy(normalize(vehicle.state['dir']))[-1]
        curr_yaw = math.degrees(vehicle.state['yaw'][0])
        last_runtime = copy.deepcopy(runtime)
        traj.append(vehicle.state['pos'])
        print(f"{kph=}")
        if runtime > 2 and abs(kph) < 0.1:
            print("Deceleration test completed, exiting...")
            final_pos = vehicle.state['pos']
            break
        bng.step(count=1)
    results = track.distance2D(final_pos, orig_pos)
    print(f"Deceleration distance={results}")
    print(f"Deceleration time={runtime - deceleration_start_time}")
    plot_trajectory(traj, addtl_trajs = [decel_traj])
    return results
...
def main():
    global default_scenario,default_spawnpoint, integral, prev_error, track
