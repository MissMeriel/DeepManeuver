...
def spawn_point(scenario_locale, road_id='default'):
    global lanewidth
    if scenario_locale == 'industrial':
        ...
        elif road_id == "7811" or road_id == "8185": # good for saliency testing
    ...
    with open(f'posefiles/qr_box_locations-{road_id}-swerve0.txt', 'r') as f:
        ...
        for i, line in enumerate(lines):
            if "platform=" in line:
                ...
                box = ScenarioObject(oid='qrbox_{}'.format(i), name='qrbox2', otype='BeamNGVehicle',
                                     pos=pos, rot=None,
                                     rot_quat=rot_quat, scale=(1, 1, 1), JBeam='qrbox2',
...
def plot_deviation(trajectories, unperturbed_traj, model, centerline, left, right, outcomes,
    ...
    # plt.legend()
    if road_id == "straight1" and "ZOOMED" not in model:
        plt.xlim([245, 335])
        plt.ylim([-123, -20])
    elif road_id == "straight1" and "ZOOMED" in model:
        plt.xlim([265, 300])
        plt.ylim([-60, -30])
    elif road_id == "curve1":
        plt.xlim([265, 300])
        ...
        plt.xlim([325, 345])
        plt.ylim([-120, -100])
    elif default_scenario == "driver_training" and road_id == "approachingfork":
        plt.xlim([-50, 55])
        plt.ylim([150, 255])
    elif road_id == "8205":
        plt.xlim([540, 640])
...
def plot_racetrack_roads(roads, bng):
    ...
    for road in roads:
        ...
        for edge in road_edges:
            if edge['middle'][0] < -250 or edge['middle'][0] > 50 or edge['middle'][1] < 300 or edge['middle'][1] > 0:
                dont_add = True
                break
            if not dont_add:
    ...
    plt.legend()
    plt.title("{} {}".format(default_scenario, road_id))
    plt.show()
    plt.pause(0.001)
...
def road_analysis(bng):
    ...
    if default_scenario == "industrial" and road_id == "racetrackstartinggate":
        edges = bng.get_road_edges('7982')
    elif default_scenario == "industrial" and road_id == "straight1":
        edges = bng.get_road_edges(args.road_id)
        adjustment_factor = 10.0
    elif default_scenario == "industrial" and road_id == "driftcourse2":
        edges = bng.get_road_edges('7987')
    elif default_scenario == "hirochi_raceway" and road_id == "startingline":
        edges = bng.get_road_edges('9096')
        edges.extend(bng.get_road_edges('9206'))
        # edges = bng.get_road_edges('9206')
    elif default_scenario == "utah" and road_id == "westhighway":
    ...
    elif default_scenario == "utah" and road_id == "westhighway2":
        ...
        # edges = bng.get_road_edges('9206')
        adjustment_factor = 1.0
    elif default_scenario == "utah" and road_id == "undef":
...
def plot_trajectory(traj, title="Trajectory", label1="AI behavior"):
    global centerline, roadleft, roadright, new_results_dir, default_scenario, road_id, qr_positions
    sp = spawn_point(default_scenario, road_id)
    x = [t[0] for t in traj]
    y = [t[1] for t in traj]
    ...
    plt.draw()
    if new_results_dir == '':
        plt.savefig("{}/{}-{}_expected-trajectory.jpg".format(os.getcwd(), default_scenario, road_id))
    else:
        plt.savefig("{}/{}-{}_expected-trajectory.jpg".format(new_results_dir, default_scenario, road_id))
    # plt.show()
    # plt.pause(1)
    plt.close("all")
...
def setup_beamng(setup_args, vehicle_model='hopper',
                 ...
                 track=default_scenario, spawnpoint=road_id):
...
def superdeepbillboard(model, sequence, direction, steering_vector, bb_size=5, iterations=400, noise_level=25,
    ...
    if dist_to_bb is not None:
        constraint = 1.0 #dist_to_bb / 22.0 #math.atan(dist_to_bb / 22.0) # 0.2 # 0.1667
        if direction == "left":
...
def run_scenario(vehicle, bng, model, spawn, direction, run_number=0,
    ...
    global centerline, road_id, unperturbed_traj, unperturbed_steer, unperturbed_seq
    starttime = time.time()
    ...
    results["unperturbed_all_ys"] = unperturbed_results['all_ys']
    outstring = f"\nRESULTS FOR DBB {model._get_name()} {road_id} {direction=} {bb_size=} {iterations=} {noise_level=}: \n" \
...
def run_scenario_superdeepbillboard(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=0, run_number=0,
    ...
    global road_id, unperturbed_traj, unperturbed_steer
    starttime = time.time()
    ...
    results['testruns_trajs'] = pert_trajs
    results['testruns_all_ys'] = Ys
    outstring = f"\nRESULTS FOR SDBB {model._get_name()} {road_id} {direction=} {bb_size=} {iterations=} {noise_level=}: \n" \
                f"Avg. deviation from expected trajectory: \n" \
                f"unperturbed:\t{results['unperturbed_deviation']}\n" \
...
def plot_billboard_ratios(runtimes, percents, detected_runtimes, detected_percents, distances, detected_distances, title):
    ...
    if "Normal" in title:
        plt.savefig("results/{}/{}-{}-normal_billboard_ratios.jpg".format(newdir, default_scenario, road_id))
    else:
        plt.savefig(
            "{}/{}-{}-collection_billboard_ratios.jpg".format(new_results_dir, default_scenario, road_id))
...
def plot_MAEs(distances, percents, detected_distances, detected_percents, unperturbed_predictions, perturbed_predictions, angleerror_distances,
    ...
    if "Normal" in title:
        ...
        plt.savefig(
            "{}/{}-{}-collection_angle_error-wrtdistance.jpg".format(new_results_dir, default_scenario, road_id))
    plt.close("all")


def run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton=40, device=torch.device('cuda')):
    global new_results_dir, steps_per_sec, expected_trajectory, unperturbed_steer, roadmiddle, unperturbed_seq
    global integral, prev_error, setpoint
    ...
    results = {'runtime': round(runtime, 3), 'damage': damage, 'kphs': kphs, 'traj': traj, 'final_img': final_img,
               ...
               "dist_to_bb": dist_to_bb, "unperturbed_seq": unperturbed_seq
               }
...
def run_scenario_with_perturbed_billboard(vehicle, bng, model, spawn, pert_billboard, run_number=0, device=torch.device('cuda'),
    ...
    global integral, prev_error, setpoint, qr_positions
...
def run_scenario_for_superdeepbillboard(vehicle, bng, model, spawn, direction, dist_to_bb_cuton=26, dist_to_bb_cutoff=26,
    ...
    global new_results_dir, centerline, expected_trajectory, qr_positions, unperturbed_seq
    global integral, prev_error, setpoint
    integral, runtime = 0.0, 0.0
    prev_error = setpoint
    model = model.to(device)
    ...
    while damage <= 0:
        ...
        if bbox_img is not None and kph > 29 and round(dist_to_bb,0) <= dist_to_bb_cuton: # and (steps % 2 < 1):
            ...
            pert_billboard, y, MAE = superdeepbillboard(model, sequence2, direction, copy.deepcopy(steering_vector2),
                                                   bb_size=bb_size, iterations=iterations, noise_level=noise_level,
...
def main():
    global new_results_dir, newdir, default_scenario, road_id, setpoint, integral
    global prev_error, centerline, centerline_interpolated, unperturbed_traj
    global steps_per_sec
    start = time.time()
    setup_args = None #get_sequence_setup(road_id)
    direction = "left"
    techniques = ["sdbb"]
    ...
    steps_per_sec = 15
    samples = 5
    newdir = "PatternExperiment-{}-road_id{}-{}".format(default_scenario, road_id, ''.join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6)))
    ...
    for cuton in cutons:
        ...
        sequence, unperturbed_results = run_scenario_to_collect_sequence(vehicle, bng, model, spawn, cuton)
        unperturbed_traj = unperturbed_results['traj']
