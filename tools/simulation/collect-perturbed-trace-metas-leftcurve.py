...
def add_qr_cubes(scenario):
    ...
    with open('posefiles/qr_box_locations-leftcurve-incorner.txt', 'r') as f:
        lines = f.readlines()
        for i, line in enumerate(lines):
            line = line.split(' ')
            pos = line[0].split(',')
        ...
        if default_scenario == "industrial" and default_spawnpoint == "leftcurve":
            # rot_quat = turn((0.0,0.0,0.9943694943814233,0.10596843229770164), -15)
            cube = ProceduralCube(name='cube_platform',
...
def main():
    ...
    steps_per_sec = 15
    samples = 50
    newdir = f'EXP1-{default_spawnpoint}-{"".join(random.choice(string.ascii_uppercase + string.digits) for _ in range(6))}'
