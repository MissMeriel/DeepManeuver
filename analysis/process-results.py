...
def get_xy_lim(topo):
    ...
    elif topo == "topo11" or topo == "scenario2":
        return [275, 305], [-90, -30]
    elif topo == "topo2" or topo == "topo21" or topo == "scenario3" or topo == "scenario4":
        # return [180, 120], [-120, -180]
        return [180, 125], [-125, -180]
    elif topo == "topo3"  or topo == "scenario5":
        return [150, 210], [-160, -100]
    elif topo == "topo31" or topo == "scenario6":
...
def generate_figures_568(resultsDir, outdir="./"):
    middle, roadleft, roadright = load_trackdef()
    pattern = re.compile("scenario[0-9]+")
...
def generate_AAE_figure(resultsDir):
    fileExt = r".pickle"
    aaes = []
    dirs = [f"{resultsDir}/{_}" for _ in os.listdir(f"{resultsDir}") if (os.path.isdir("/".join([resultsDir, _])) and "deepmaneuver" in _)]
    for d in dirs:
        rundirs = [f"{d}/{_}" for _ in os.listdir(d) if os.path.isdir(f"{d}/{_}")]
        for rd in rundirs:
            results_files = [_ for _ in os.listdir(rd) if _.endswith(fileExt)]
            for file in results_files:
                results = unpickle_results("/".join([rd, file]))
...
if __name__ == '__main__':
    ...
    if args.figure == "table1": # study table 1
        ...
        resultsDir = args.dataset + "/study/table1/scenario1"
        generate_AAE_figure(resultsDir)
